...
class DHWC_Ajax_Widget_Product_Brand_Filter extends DHWC_Ajax_Widget {
	
	public function __construct(){
		$this->widget_cssclass    = 'woocommerce widget_layered_nav widget_category_filter dhwc_ajax_product_brand_filter';
        $this->widget_description = __('Display a list of brands to filter products in your store.', 'dhwc-ajax');
        $this->widget_id          = 'dhwc_ajax_product_brand_filter';
        $this->widget_name        = __('DHWC Ajax Product Brand Filter', 'dhwc-ajax');
		parent::__construct();
	}
	...
	public function init_settings() {
		$this->settings = array(
			'title' => array(
				'type'  => 'text',
				'std'   => __( 'Filter Brands', 'dhwc-ajax' ),
	...
	protected function get_filtered_term_product_counts( $term_ids, $taxonomy, $query_type ) {
		...
		$query             = apply_filters( 'dhwc_ajax_brand_filtered_counts_query', $query );
		$query             = implode( ' ', $query );
	
		// We have a query - let's see if cached results of this query already exist.
		$query_hash    = md5( $query );
		$cached_counts = (array) get_transient( 'dhwc_ajax_brand_filtered_counts' );
	
		if ( ! isset( $cached_counts[ $query_hash ] ) ) {
			$results                      = $wpdb->get_results( $query, ARRAY_A ); // @codingStandardsIgnoreLine
			$counts                       = array_map( 'absint', wp_list_pluck( $results, 'term_count', 'term_count_id' ) );
			$cached_counts[ $query_hash ] = $counts;
			set_transient( 'dhwc_ajax_brand_filtered_counts', $cached_counts, DAY_IN_SECONDS );
		}
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type, $show_count ) {
		...
		$filter_name    	= dhwc_ajax_get_brand_filter_query();
		foreach ( $terms as $term ) {
			...
			if ( $count > 0 || $option_is_set ) {
				$link      = esc_url( apply_filters( 'dhwc_ajax_brand_filter_link', $link, $term, $taxonomy ) );
			...
			if($show_count){
				$term_html .= ' ' . apply_filters( 'dhwc_ajax_brand_filter_count', '<span class="count">' . absint( $count ) . '</span>', $count, $term );
			}
			echo '<li ' . ( $option_is_set ? 'class="chosen"' : '' ) . '>';
			echo apply_filters( 'dhwc_ajax_brand_filter_html', $term_html, $term, $link, $count );
			echo '</li>';
		}
...
add_action('widgets_init', function(){
	return register_widget("DHWC_Ajax_Widget_Product_Brand_Filter");
});