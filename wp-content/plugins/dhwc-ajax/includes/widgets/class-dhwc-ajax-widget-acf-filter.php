...
class DHWC_Ajax_Widget_ACF_Filter extends DHWC_Ajax_Widget {
	...
	public function widget( $args, $instance ) {
		...
		foreach ((array)$field['choices'] as $value => $label){
		    $count = absint($this->get_filtered_product($field_name, $value, $field));
			...
			if((string)$filtered === (string) $value){
				...
				echo '<li class="chosen"><a href="'.$link.'">'.$label.$count_html.'</a></li>';
			}else{
				$link = add_query_arg( array($filter_name => $value,'acf'=>'1' ),$link);
				echo '<li><a href="'.$link.'">'.$label.$count_html.'</a></li>';
			}
	...
	protected function get_filtered_product($field_name, $value, $field){
		global $wpdb;
		...
		$sql = apply_filters( 'dhwc_ajax_widget_acf_filter_sql', $sql, $meta_query_sql, $tax_query_sql );
